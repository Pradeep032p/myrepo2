...
async def next_page(bot, query):
    ...
    btn.insert(0, [
        InlineKeyboardButton("🔹 How to download 🔹", url='https://t.me/DM_backups/43')
    ])
    try:
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🤖 Updates', url='https://t.me/DM_backups')
        ], [
...
async def auto_filter(client, msg, spoll=False):
    ...
    btn.insert(0, [
        InlineKeyboardButton("🔹 How to download 🔹", url='https://t.me/DM_backups/64')
    ])
    ...
    if imdb and imdb.get('poster'):
        try:
            bh = await message.reply_photo(photo=imdb.get('poster'), caption=cap[:1024],reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(300)
            await bh.delete()
        except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty):
            pic = imdb.get('poster')
            poster = pic.replace('.jpg', "._V1_UX360.jpg")
            bhh = await message.reply_photo(photo=poster, caption=cap[:1024], reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(300)
            await bhh.delete()
        except Exception as e:
            logger.exception(e)
            bbh = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(300)
            await bbh.delete()
    else:
        mbh = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
        await asyncio.sleep(300)
        await mbh.delete()
    if spoll:
